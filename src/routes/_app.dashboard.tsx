...
export const Route = createFileRoute("/_app/dashboard")({
  loader: async () => {
    const [{ data: q }, { data: m }] = await Promise.all([
      supabase
        .from("quotations")
        .select("id, quotation_number, total_amount, status, created_at, customer_id")
        .order("created_at", { ascending: false })
        .limit(50),
      supabase.from("purifier_models").select("id").eq("active", true),
    ]);
    return { quotations: q ?? [], modelCount: m?.length ?? 0 };
  },
  component: Dashboard,
});
...
function Dashboard() {
  const { quotations, modelCount } = Route.useLoaderData();
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Welcome back. Here&apos;s a snapshot of your quotation activity.
          </p>
        </div>
            ...
            <div className="divide-y">
              {quotations.slice(0, 8).map((q) => (
                <Link
                  ...
                  to="/quotations/$id"
                  params={{ id: q.id }}
                  className="flex items-center justify-between gap-4 py-3 transition hover:bg-accent/40 -mx-2 px-2 rounded-md"
                >
                  ...
                  <div className="flex items-center gap-4">
                    <Badge className={statColor(q.status)} variant="secondary">
                      {q.status}
                    </Badge>
                    <div className="font-mono text-sm font-semibold">
                      {formatINR(Number(q.total_amount))}
                    </div>
                  </div>
                </Link>
              ))}
